...
- **Stable history and scope-aware caching.** Cancelling a preview preserves both app and window recency. Cached previews survive display, Spaces, and minimized-window filters; idle captures still follow the current scope, and closed/excluded windows are cleaned up within the existing cache limits.
